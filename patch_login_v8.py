...
# 1. Login V8: HTML Layout for Header/Logo (Guaranteed Tightness)
refined_login_code_v8 = """
...
if not st.session_state['user'] and not st.session_state.get('force_logout'):
    ...
    st.markdown('''
        ...
        /* 3. INPUT STYLING */
        /* Clean, rounded, minimal height */
        .stTextInput {
            margin-bottom: -15px !important; /* Gaps between inputs */
        }
        ...
        div[data-testid="stButton"] > button[kind="primary"] {
            ...
            font-weight: 700;
            font-size: 1rem;
            margin-top: 20px; /* Spacer from inputs */
        }
    ...
    with c_form:
        ...
        # This guarantees they are stuck together no matter what Streamlit does
        ...
        # --- INPUTS ---
        # No extra st.write spacers!
        email = st.text_input("Email", key="login_email", placeholder="Email", label_visibility="collapsed")
        
        # Using a tiny markdown to nudge if absolutely needed, but trying margin-bottom CSS first
...
if start_idx != -1 and end_idx != -1:
    print(f"Replacing Login V8 at lines {start_idx+1}-{end_idx}")
    
    new_lines = []
    # Lines before
    new_lines.extend(lines[:start_idx])
    # New Code
    new_lines.append(refined_login_code_v8 + '\n')
    # Lines after
    ...
    print("SUCCESS: Login Page Refined V8 (HTML Block Solution).")
else:
