...
# 1. Login V5: Centered, Big Logo, Perfect Match
...
if not st.session_state['user'] and not st.session_state.get('force_logout'):
    ...
    with c_form:
        ...
        # We use a container w/ adjustments or raw html for centering image if streamlt layout acts up
        # But st.image centers if width is small, let's use columns for centering or CSS
        ...
        # Using markdown with a button simulation or actual button
        # The user wants "Don't have an account? Sign up" centered.
...
if start_idx != -1 and end_idx != -1:
    print(f"Replacing Login V5 at lines {start_idx+1}-{end_idx}")
    ...
    print("SUCCESS: Login Page Refined V5 (Centered & Precise).")
else:
