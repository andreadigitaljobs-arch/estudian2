...
def render_library_v2(assistant, key_suffix="default"):
    """
    ...
    */
    ...
    with t_c1:
        if st.button("📂 Raíz", key=f"lib_btn_root_{key_suffix}", use_container_width=True, help="Ir a la carpeta principal"):
    ...
    with t_c2:
        if st.button("📤 Subir", key=f"lib_btn_upload_{key_suffix}", type=get_type('upload'), use_container_width=True, help="Subir archivos o crear notas"):
            set_tool('upload')
            st.rerun()

    with t_c3:
        if st.button("✨ Nueva", key=f"lib_btn_new_{key_suffix}", type=get_type('create'), use_container_width=True, help="Crear nueva carpeta"):
            set_tool('create')
            st.rerun()

    with t_c4:
        if st.button("⚙️ Gestión", key=f"lib_btn_manage_{key_suffix}", type=get_type('manage'), use_container_width=True, help="Renombrar o borrar carpetas"):
            set_tool('manage')
            st.rerun()

    with t_c5:
        if st.button("🔍 Buscar", key=f"lib_btn_search_{key_suffix}", type=get_type('search'), use_container_width=True, help="Buscar en toda la biblioteca"):
            set_tool('search')
            st.rerun()
            
    with t_c6:
         if st.button("🧹 Duplicados", key=f"lib_btn_dupes_{key_suffix}", type=get_type('duplicates'), use_container_width=True, help="Buscar archivos duplicados"):
            set_tool('duplicates')
            st.rerun()

    with t_c7:
         if st.button("📦 Backup", key=f"lib_btn_backup_{key_suffix}", type=get_type('backup'), use_container_width=True, help="Descargar todo"):
            set_tool('backup')
            st.rerun()
