    ...
    if subfolders:
        ...
        for i, unit in enumerate(subfolders):
            with f_cols[i % 3]:
                ...
                components.html(f"""<div onclick="window.parent.postMessage({{type: 'streamlit:setComponentValue', value: '{unit_id}'}}, '*')" style="background: transparent; border: none; padding: 16px 8px; text-align: center; cursor: pointer; transition: all 0.2s ease; min-height: 180px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 12px;" onmouseover="this.style.backgroundColor='rgba(0,0,0,0.04)'; this.style.borderRadius='12px';" onmouseout="this.style.backgroundColor='transparent'; this.style.borderRadius='0px';"><div style="font-size: 80px; line-height: 1;">📁</div><div style="color: #1e293b; font-size: 14px; font-weight: 700; line-height: 1.3; font-family: 'Segoe UI', system-ui, sans-serif; max-width: 180px; word-wrap: break-word;">{unit_name}</div><div style="color: #64748b; font-size: 12px; font-weight: 500;">{count} archivos</div></div>""", height=200, key=f"fdir_{unit_id}")
