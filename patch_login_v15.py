...
# 1. Login V15: Flexbox Header Control
# - Using Flexbox to enforce 10px gap strictly between Logo and Title.
refined_login_code_v15 = '''
...
if not st.session_state['user'] and not st.session_state.get('force_logout'):
    ...
    st.markdown("""
        ...
        .header-flex {
            ...
            gap: 10px; /* EXACT 10px SPACING REQUESTED */
            margin-bottom: 25px;
        }
        .login-logo {
            width: 100%;
...
if start_line != -1 and end_line != -1:
    print(f"Applying Login V15 to lines {start_line+1}-{end_line}...")
    
    new_lines = []
    # Lines before
    new_lines.extend(lines[:start_line])
    # New Code
    new_lines.append(refined_login_code_v15 + '\\n')
    # Lines after
    new_lines.extend(lines[end_line:])
    
    with open(app_path, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("SUCCESS: Login V15 Applied (Strict CSS Flexbox + 10px Gap).")
else:
