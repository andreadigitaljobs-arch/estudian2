...
# 1. Login V12: V10 Base (Good) + Larger Logo (Requested)
refined_login_code_v12 = """
...
if not st.session_state['user'] and not st.session_state.get('force_logout'):
    ...
    # --- CUSTOM CSS FOR LOGIN PAGE V12 ---
    st.markdown('''
        <style>
        /* 1. LAYOUT RESET & COMPACTING */
        ...
        /* 2. PULL CONTENT UP VS STREAMLIT PADDING */
        div[data-testid="stVerticalBlock"] {
            margin-top: -30px; 
            gap: 0.5rem !important;
        }

        /* 3. INPUTS - Clean Professional */
        ...
        /* 5. SECONDARY BUTTON (Sign up Link) */
    ...
    with c_form:
        # --- HEADER ---
        # Increased size to 240px (was 180px in V10)
        # Margin Bottom 15px (Balanced)
        logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="width: 240px; margin-bottom: 15px;">' if logo_b64 else '<h2>e-education</h2>'
        ...
        with c_foot_1:
             # Using DOUBLE QUOTES for the python string to safely contain the single quote in "Don't"
...
if start_idx != -1 and end_idx != -1:
    print(f"Replacing Login V12 at lines {start_idx+1}-{end_idx}")
    
    new_lines = []
    # Lines before
    new_lines.extend(lines[:start_idx])
    # New Code
    new_lines.append(refined_login_code_v12 + '\n')
    # Lines after
    ...
    print("SUCCESS: Login Page Refined V12 (V10 Layout + 240px Logo).")
else:
