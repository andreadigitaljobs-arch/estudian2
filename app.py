...
@st.cache_resource
def get_transcriber_engine(key, model_choice="gemini-2.0-flash", breaker="V5"):
    return Transcriber(key, model_name=model_choice, cache_breaker=breaker)

@st.cache_resource
def get_assistant_engine(key, model_choice="gemini-2.0-flash", breaker="V5"):
...
if api_key:
    try:
        ...
        transcriber = get_transcriber_engine(api_key, model_choice="gemini-2.0-flash", breaker="V5")
        assistant = get_assistant_engine(api_key, model_choice="gemini-2.0-flash", breaker="V5")
    except Exception as e:
