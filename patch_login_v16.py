...
# 1. Login V16: Negative Margins for Logo Gap
# - The user's logo image likely has whitespace at the bottom.
# - We will use negative margins to force the title UP into that whitespace.
refined_login_code_v16 = '''
...
if not st.session_state['user'] and not st.session_state.get('force_logout'):
    ...
    st.markdown("""
        ...
        /* 6. TYPOGRAPHY & HEADER */
        /* Flexbox Header for Strict Spacing */
        .header-flex {
            display: flex;
            flex-direction: column;
        ...
        .login-logo {
            ...
            object-fit: contain;
            display: block;
            margin-bottom: -50px; /* AGGRESSIVE NEGATIVE MARGIN: Pulls content up */
            position: relative;
            z-index: 1; /* Keep logo above title if they overlap unintentionally */
        }
        .login-title {
    ...
    with c_form:
        ...
        st.markdown(f"""
            <div class="header-flex">
                ...
                <div style="margin-top: 0px;"> <!-- wrapper to catch negative margin -->
...
if start_line != -1 and end_line != -1:
    print(f"Applying Login V16 to lines {start_line+1}-{end_line}...")
    
    new_lines = []
    # Lines before
    new_lines.extend(lines[:start_line])
    # New Code
    new_lines.append(refined_login_code_v16 + '\\n')
    # Lines after
    new_lines.extend(lines[end_line:])
    
    with open(app_path, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    print("SUCCESS: Login V16 Applied (Negative Margins).")
else:
