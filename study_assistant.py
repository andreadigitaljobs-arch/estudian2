...
*Este mensaje es automático del sistema de protección de costos.*"""
    ...
    def refine_quiz_results(self, original_results, chat_history):
        ...
        for msg in chat_history:
            role = "ESTUDIANTE" if msg['role'] == "user" else "PROFESOR (T�a)"
            history_str += f"{role}: {msg['content']}\n"
            
        prompt = f"""
        ACT�aA COMO UN AUDITOR DE EXÁMENES.
        ...
        INSTRUCCIONES CLAVE:
        1. Tu única misión es GENERAR LA VERSI� N FINAL Y CORRECTA de los resultados.
        2. Analiza el Chat.
           - Si el Profesor (Tú) ADMITI�  un error, CAMBIA esa respuesta en la lista.
           - Si el Profesor explicó que la respuesta original estaba bien, MANT�0 N la original.
           - Si no se habló de una preguntas, MANT�0 N la original intacta.
